...
def prepare_soil(site_id: int) -> pd.DataFrame:
    ...
    for col in MOISTURE_SOURCE_COLS:
        soil[col] = pd.to_numeric(soil[col], errors="coerce") * PCT_TO_MM_4IN
...
def main() -> None:
    ...
    write_outputs(events_df, loss_points_df, statuses)
