...
def read_station_data(site_id: int) -> pd.DataFrame:
    ...
    for source, target in MOISTURE_RENAME.items():
        soil[target] = pd.to_numeric(soil[source], errors="coerce") * PCT_TO_MM_4IN
...
def pick_representative_event(events: pd.DataFrame) -> pd.Series:
    pool = events[
        ...
        & (events["trim3_r2"] >= 0.9)
        & (events["duration_h"].between(5.0, 14.0))
        & (events["total_drop_mm"].between(1.0, 3.0))
        & (events["rain_lag_h"].between(0.25, 24.0))
    ].copy()
    if pool.empty:
        pool = events[(events["layer"] == "moisture_4in") & (~events["interrupted_by_rain"])].copy()
    med_duration = events.loc[events["layer"] == "moisture_4in", "duration_h"].median()
    med_drop = events.loc[events["layer"] == "moisture_4in", "total_drop_mm"].median()
    pool["score"] = (
        (pool["duration_h"] - med_duration).abs() / med_duration
        + (pool["total_drop_mm"] - med_drop).abs() / med_drop
        + (pool["rain_lag_h"].fillna(24.0) / 24.0) * 0.25
    )
    return pool.sort_values("score").iloc[0]
...
def plot_representative_event(ax: plt.Axes, events: pd.DataFrame) -> pd.Series:
    ...
    window = merged.loc[start - pd.Timedelta(hours=14) : end + pd.Timedelta(hours=8), [layer, "Rain"]].copy()
    window["elapsed_h"] = (window.index - start).total_seconds() / 3600.0

    ax.plot(window["elapsed_h"], window[layer], color=COLORS["soil"], lw=1.4)
    ax.axvspan(0, (end - start).total_seconds() / 3600.0, color="#DDE8F1", alpha=0.7, lw=0)
    ax.axvline(0, color=COLORS["accent"], lw=1.0)
    ax.axvline((end - start).total_seconds() / 3600.0, color=COLORS["neutral_mid"], lw=1.0, ls="--")
    ax.scatter([0], [event["start_mm"]], s=20, color=COLORS["accent"], zorder=5)
    ax.scatter(
    ...
    ax.set_xlabel("Hours from detected SMDE start")
    ...
    ax.text(
        0.02,
        0.04,
        f"duration {event['duration_h']:.1f} h; drop {event['total_drop_mm']:.2f} mm; rain lag {event['rain_lag_h']:.1f} h",
        transform=ax.transAxes,
        ha="left",
        va="bottom",
        fontsize=6.4,
...
def plot_funnel(ax: plt.Axes, funnel: pd.DataFrame) -> None:
    ...
    ax.set_title("Detection-to-construction subset funnel", loc="left", fontsize=8)
...
def plot_site_layer_heatmap(ax: plt.Axes, site_heat: pd.DataFrame) -> None:
    ...
    ax.set_title("Clean rainfall-associated rate by station-layer", loc="left", fontsize=8)
    ...
    for i in range(heat.shape[0]):
        for j in range(heat.shape[1]):
            ...
            if np.isfinite(value) and n >= 120:
                color = "white" if value > 0.58 else COLORS["neutral_dark"]
                ax.text(j, i, f"{value:.0%}", ha="center", va="center", fontsize=4.8, color=color)
...
def write_report(events: pd.DataFrame, tables: dict[str, pd.DataFrame], representative: pd.Series) -> None:
    ...
    report = [
        ...
        f"- Clean rainfall-associated and stage-II-like: {int(funnel.loc[funnel['step'] == 'Clean + stage-II-like', 'events'].iloc[0]):,} ({float(funnel.loc[funnel['step'] == 'Clean + stage-II-like', 'fraction_of_detected'].iloc[0]):.1%}).",
        "",
        "## Layer summary",
        "",
    ...
    for row in layer_summary.itertuples(index=False):
        report.append(
            f"| {row.layer_label} | {row.events:,} | {row.associated_48h_rate:.1%} | {row.clean_48h_rate:.1%} | {row.stageII_clean_48h_rate:.1%} |"
        )
    report.extend(
        [
            "",
